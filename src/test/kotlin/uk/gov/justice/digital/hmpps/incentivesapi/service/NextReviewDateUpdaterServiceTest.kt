...
class NextReviewDateUpdaterServiceTest {
  ...
  private val nextReviewDateUpdaterService =
  ...
  inner class UpdateAllTest {
    ...
    @Test
    fun `updateMany() when no reviews in database`(): Unit =
      runBlocking {
    ...
    @Test
    fun `updateMany() when some reviews in database`(): Unit =
      runBlocking {
        val offender2Reviews =
          flowOf(
  ...
  @Test
  fun `update() calculate, save and return the next review date`(): Unit =
    runBlocking {
      val bookingId = 1234L
