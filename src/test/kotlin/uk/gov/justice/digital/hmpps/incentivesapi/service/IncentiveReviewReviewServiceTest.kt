...
class IncentiveReviewReviewServiceTest {
  ...
  private val prisonerIncentiveReviewService =
    PrisonerIncentiveReviewService(
      prisonApiService,
  ...
  inner class AddCreateIncentiveReviewRequest {
    ...
    private val prisonerInfo =
      prisonerAtLocation(
        bookingId = bookingId,
        offenderNo = prisonerNumber,
      )
    private val createIncentiveReviewRequest =
      CreateIncentiveReviewRequest(
        iepLevel = "ENH",
        comment = "A review took place",
        reviewType = ReviewType.REVIEW,
      )
    private val incentiveReview =
      IncentiveReview(
    ...
    @Test
    fun `addIepReview() by prisonerNumber`(): Unit =
      runBlocking {
        // Given
    ...
    @Test
    fun `addIepReview() by bookingId`(): Unit =
      runBlocking {
        // Given
    ...
    @Test
    fun `update multiple IepLevels with current flag`(): Unit =
      runBlocking {
        // Given
        whenever(prisonApiService.getPrisonerInfo(bookingId)).thenReturn(prisonerInfo)
  ...
  inner class ProcessReceivedPrisoner {
    ...
    @Test
    fun `process 'prisoner alerts updated' event when ACCT alert added`(): Unit =
      runBlocking {
        val bookingId = 1234567L

        // Given
        val prisonerAlertsUpdatedEvent =
    ...
    @Test
    fun `process 'prisoner alerts updated' event when ACCT alert removed`(): Unit =
      runBlocking {
        val bookingId = 1234567L
    ...
    @Test
    fun `process 'prisoner alerts updated' event when alerts didn't change`(): Unit =
      runBlocking {
        val bookingId = 1234567L

        // Given
        val prisonerUpdatedEvent =
          prisonerAlertsUpdatedEvent(
    ...
    fun `do not process irrelevant reasons`(reason: String): Unit =
      runBlocking {
        // When
    ...
    @Test
    fun `process MERGE event`(): Unit =
      runBlocking {
        // Given - default for that prison is Enhanced
        val prisonMergeEvent = prisonMergeEvent()
        val prisonerAtLocation =
          prisonerAtLocation(
            bookingId = 1234567,
            offenderNo = "A1244AB",
          )
    ...
    @Test
    fun `do not create IEP level if prisonerNumber is null`(): Unit =
      runBlocking {
        // Given
  ...
  inner class DeleteIncentiveRecordTest {
    ...
    private val incentiveReviewDetail =
    ...
    @Test
    fun `deletes the IEP review`(): Unit =
      runBlocking {
        // When
    ...
    @Test
    fun `updates the next review date of the affected prisoner`(): Unit =
      runBlocking {
        // When
        prisonerIncentiveReviewService.deleteIncentiveRecord(bookingId, incentiveRecord.id)
      }
    ...
    @Test
    fun `sends IepReview event and audit message`(): Unit =
      runBlocking {
  ...
  inner class UpdateIncentiveRecordTest {
    ...
    private val iepLevelDescription = "Enhanced"
    private val reviewTime = LocalDateTime.now().minusDays(10)
    private val update: IncentiveRecordUpdate =
      IncentiveRecordUpdate(
        comment = "UPDATED",
        reviewTime = null,
        current = null,
      )
    ...
    @Test
    fun `updates the IEP review`(): Unit =
      runBlocking {
        // When
        val result =
    ...
    @Test
    fun `updates the next review date for the prisoner`(): Unit =
      runBlocking {
        // When
        prisonerIncentiveReviewService.updateIncentiveRecord(bookingId, incentiveRecord.id, update)
      }
    ...
    @Test
    fun `If request has current true we update the previous IEP Level with current of true`(): Unit =
      runBlocking {
        // Given
  ...
  private fun prisonOffenderEvent(
    reason: String,
    ...
    HMPPSDomainEvent(
      ...
      occurredAt = Instant.now(),
      description =
        "A prisoner has been received into a prison with reason: " +
          when (reason) {
  ...
  private val globalIncentiveLevels =
    listOf(
