...
class IncentiveReviewReviewServiceTest {
  ...
  inner class GetIncentiveReviewHistory {
    ...
    fun `will not return iep details if withDetails is false`(): Unit = runBlocking {
      ...
      // When
      val result = prisonerIncentiveReviewService.getPrisonerIncentiveHistory(bookingId, withDetails = false)
  ...
  inner class UpdateIncentiveRecordTest {
    ...
    fun `updates the IEP review`(): Unit = runBlocking {
      // When
      val result = prisonerIncentiveReviewService.updateIncentiveRecord(bookingId, incentiveRecord.id, update)
