...
class OffenderSearchServiceTest {
  ...
  @Test
  fun `only one page of results is loaded if it is the last`(): Unit =
    runBlocking {
      // mocks 1 page with 1 result
      val webClient =
        createWebClientMockResponses(
          listOf(
            OffenderSearchPrisoner(
              prisonerNumber = "A1409AE",
              bookingId = 110001,
  ...
  @Test
  fun `all pages of results are loaded`(): Unit =
    runBlocking {
