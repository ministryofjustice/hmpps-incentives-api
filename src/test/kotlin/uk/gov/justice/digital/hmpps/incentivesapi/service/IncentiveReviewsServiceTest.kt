...
class IncentiveReviewsServiceTest {
  ...
  @Test
  fun `maps responses from offender search`(): Unit =
    runBlocking {
      val offenders =
        listOf(
  ...
  @Test
  fun `maps case notes from prison api`(): Unit =
    runBlocking {
      // Given
  ...
  @Test
  fun `defaults positive and negative behaviours to 0 if no case notes returned`(): Unit =
    runBlocking {
      // Given
      val prisonerNumber = "G6123VU"
  ...
  @Test
  fun `filters reviews by level`(): Unit =
    runBlocking {
      val offenders =
        listOf(
  ...
  inner class SortingAndFiltering {
    ...
    private val nextReviewDatesMap =
      mapOf(
        110001L to LocalDate.now(clock).plusYears(1),
    ...
    @Test
    fun `oldest date of next review is first by default`(): Unit =
      runBlocking {
        // Given
        whenever(prisonApiService.getLocation(any())).thenReturnLocation("MDI-2-1")
        val offenders =
    ...
    @Test
    fun `can reverse order by date of next review`(): Unit =
      runBlocking {
        // Given
        whenever(prisonApiService.getLocation(any())).thenReturnLocation("MDI-2-1")
        val offenders =
    ...
    @Test
    fun `can sort by non-default parameters`(): Unit =
      runBlocking {
        // Given
        whenever(prisonApiService.getLocation(any())).thenReturnLocation("MDI-2-1")
        val offenders =
        ...
        // When
        val reviews =
          incentiveReviewsService.reviews(
            "MDI",
            "MDI-2-1",
            "STD",
  ...
  @Test
  fun `throw exception if cannot find incentive level one bookingId`(): Unit =
    runBlocking {
  ...
  @Test
  fun `throw exception if cannot find incentive levels all bookingIds`(): Unit =
    runBlocking {
  ...
  @Test
  fun `overdue count where 2 next review are in the past`(): Unit =
    runBlocking {
      // Given
      whenever(prisonApiService.getLocation(any())).thenReturnLocation("MDI-2-1")
      val offenders =
  ...
  @Test
  fun `overdue count where 3 next review are in the past but not all in page of results`(): Unit =
    runBlocking {
      // Given
      whenever(prisonApiService.getLocation(any())).thenReturnLocation("MDI-2-1")
      val offenders =
  ...
  @Test
  fun `overdue count where no next reviews are in the past`(): Unit =
    runBlocking {
      // Given
      whenever(prisonApiService.getLocation(any())).thenReturnLocation("MDI-2-1")
      whenever(offenderSearchService.getOffendersAtLocation(any(), any())).thenReturn(
